...
select s.product_name, s.sale_date, s.amount, 
sum(s.amount) over (partition by s.product_name order by s.sale_date) as cumulative_sales
from sales as s
...
select s.product_name, s.sale_date, s.amount, 
s.amount - LAG(s.amount) OVER (PARTITION BY s.product_name ORDER BY s.sale_date) AS sales_diff
from sales as s)

select 
product_name, max(sales_diff) as max_sales_diff, min(sales_diff) as min_sales_diff
